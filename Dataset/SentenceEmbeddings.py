...
class SentenceEmbeddingsGraphAbstractTwo(BaseSentenceEmbeddings):
    ...
    def __getitem__(self, idx):
        ...
        emb1spec = torch.from_numpy(
            self.embeddingsSpecter[int(self.X[idx, 0])])
        emb2spec = torch.from_numpy(
            self.embeddingsSpecter[int(self.X[idx, 1])])
        ...
        concatenated_embeddings = torch.cat(
            (emb1spec, emb2spec, emb1sci, emb2sci, torch.Tensor(self.X[idx, 2:])), dim=0)
